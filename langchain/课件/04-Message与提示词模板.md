...
大模型没有记忆，它的输出只和输入模型的内容有关（上下文）。
**如果应用需要“记住”对话历史，需要在程序中维护消息列表。**

![消息列表维护机制示意图](assets/04-page-01-image-01.png)

**在 LangChain 中，Message（消息）是模型交互的最基本单元**。它既代表模型接收到的`输入（Input）`，也代表模型生成的`输出（Output）`。

与大模型的对话，都由多条 Message 构成。
Message 不仅包含`文字内容`，还携带描述上下文状态的`元信息（metadata）`，用于保持对话的一致性和可追踪性。

LangChain 1.0 中提供了跨模型统一的 Message 标准**： 无论你使用的是什么模型，这一标准都能保持一致的行为。
好处：
- `兼容性强`：不同模型的消息格式自动对齐。
...
LangChain定义了很多消息类型，是通过`role` 区分。
